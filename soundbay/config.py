Here is your full file with **all built-in generic types replaced by `typing` equivalents** (`list[str] → List[str]`, `tuple[int, int] → Tuple[int, int]`).
...
def merge_configs(base_config: TrainingConfig, override_config: TrainingConfig) -> TrainingConfig:
    return OmegaConf.to_object(
        OmegaConf.merge(
            OmegaConf.structured(base_config),
            OmegaConf.structured(override_config)
        )
    )
