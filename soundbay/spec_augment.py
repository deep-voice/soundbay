"""SpecAugment augmentations for spectorgrams.

Credit to GoogleBrain's SpecAugment: A Simple Data Augmentation Method for Automatic Speech Recognition (https://arxiv.org/abs/1904.08779)
Credit to https://github.com/zcaceres/spec_augment for pytorch implementation.
"""
import torch
import random
...
class TimeMask(nn.Module):
    def __init__(self, T=40, num_masks=1, replace_with_zero=False, p=0.9):
    ...
    def forward(self, spec):
        if random.random() < self.p:
            return time_mask(spec, self.T, self.num_masks, self.replace_with_zero)
        return spec


class FreqMask(nn.Module):
    def __init__(self, F=30, num_masks=1, replace_with_zero=False, p=0.9):
    ...
    def forward(self, spec):
        if random.random() < self.p:
            return freq_mask(spec, self.F, self.num_masks, self.replace_with_zero)
        return spec


class TimeWarp(nn.Module):

    def __init__(self, W=10, p=0.9):
